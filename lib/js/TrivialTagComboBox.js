    ...
    var TrivialTagComboBox = (function () {
        function TrivialTagComboBox(originalInput, options) {
            ...
            this.config = $.extend({
                ...
                selectionAcceptor: function (e) { return !e._isFreeTextEntry; }
            }, options);
            ...
            this.$editor.appendTo(this.$tagArea).addClass("tr-tagbox-editor tr-editor")
                ...
                if (_this.blurCausedByClickInsideComponent) {
                    ...
                    _this.entries = null;
                    _this.closeDropDown();
                    if (_this.$editor.text().trim().length > 0) {
                        _this.addSelectedEntry(_this.config.freeTextEntryFactory(_this.$editor.text()), true, e);
                    }
                ...
                if (TrivialCore_1.keyCodes.isModifierKey(e)) {
                    ...
                    if (_this.isDropDownOpen && highlightedEntry != null) {
                        _this.addSelectedEntry(highlightedEntry, true, e);
                        e.preventDefault();
                    }
                    else if (_this.$editor.text().trim().length > 0) {
                        _this.addSelectedEntry(_this.config.freeTextEntryFactory(_this.$editor.text()), true, e);
                        e.preventDefault();
                    }
                ...
                if (editorValueBeforeCursor.length > 0) {
                    ...
                    for (var i = 0; i < tagValuesEnteredByUser.length - 1; i++) {
                        ...
                        if (value.length > 0) {
                            _this.addSelectedEntry(_this.config.freeTextEntryFactory(value), true, e);
                        }
            ...
            this.listBox.onSelectedEntryChanged.addListener(function (selectedEntry, eventSource, originalEvent) {
                if (selectedEntry) {
                    _this.addSelectedEntry(selectedEntry, true, originalEvent);
        ...
        TrivialTagComboBox.prototype.query = function (highlightDirection) {
            ...
            this.config.queryFunction(this.getNonSelectedEditorValue(), function (newEntries) {
        ...
        TrivialTagComboBox.prototype.addSelectedEntry = function (entry, fireEvent, originalEvent, forceAcceptance) {
            var _this = this;
