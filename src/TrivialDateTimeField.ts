	...
	constructor(originalInput: JQuery | Element | string, options: TrivialDateTimeFieldConfig = {}) {
		...
		this.$dateTimeField = $(`<div class="tr-datetimefield tr-input-wrapper">
            <div class="tr-editor-wrapper">
                <div class="tr-date-icon-wrapper"/>
                <div class="tr-date-editor" contenteditable="true"/>
                <div class="tr-time-icon-wrapper"/>
                <div class="tr-time-editor" contenteditable="true"/>
            </div>
				...
				(e: KeyboardEvent) => {
					if (keyCodes.isModifierKey(e)) {
						...
						if (this.getActiveEditor() === this.$timeEditor && e.which == keyCodes.left_arrow && window.getSelection().focusOffset === 0) {
							e.preventDefault();
							selectElementContents(this.$dateEditor[0], 0, this.$dateEditor.text().length);
						} else if (this.getActiveEditor() === this.$dateEditor && e.which == keyCodes.right_arrow && window.getSelection().baseOffset === window.getSelection().focusOffset && window.getSelection().focusOffset === this.$dateEditor.text().length) {
							e.preventDefault();
							selectElementContents(this.$timeEditor[0], 0, this.$timeEditor.text().length);
						}
